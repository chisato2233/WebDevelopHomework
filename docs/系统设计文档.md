...
| TypeScript | 5+ | 类型安全 |
| Tailwind CSS | 3+ | 样式框架 |
| **shadcn/ui** | 最新 | UI组件库（基于 Radix UI） |
| Axios | 最新 | HTTP请求 |
...
**关于 shadcn/ui 的说明**：
- shadcn/ui 是基于 Radix UI 和 Tailwind CSS 的组件库
- 组件代码直接复制到项目中，便于定制和修改
- 提供现代化、美观的 UI 风格
- 与 Tailwind CSS 深度集成，样式一致性好
- 安装方式：`npx shadcn@latest init` 初始化，`npx shadcn@latest add <component>` 添加组件
...
npx shadcn@latest add table tabs badge avatar
npx shadcn@latest add dropdown-menu navigation-menu
npx shadcn@latest add form  # 包含 react-hook-form 集成
```
